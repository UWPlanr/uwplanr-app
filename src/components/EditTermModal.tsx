...
const EditTermModal = ({ term }: Props) => {
  ...
  return (
    <>
        <button className="btn btn-circle btn-ghost" onClick={() => (document.getElementById(`edit-modal-${term.term}`) as HTMLDialogElement).showModal()}><Edit /></button>
        <dialog id={`edit-modal-${term.term}`} className="modal">
